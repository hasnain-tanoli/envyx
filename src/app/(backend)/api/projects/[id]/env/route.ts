...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        await db.insert(env_audit_log).values({
            ...
            action: 'create',
            new_value: value, // Log plaintext for audit log if that's desired, or we can log encrypted. Let's log plaintext for audit history if secure enough.
        });

        // Update env_count
        await db.update(projects).set({ env_count: (project.env_count || 0) + 1 }).where(eq(projects.id, id));

        return NextResponse.json({ ...newEnv, value }); // Return plaintext value to UI after creation
    } catch (e: unknown) {
        return NextResponse.json({ error: e instanceof Error ? e.message : 'Internal Server Error' }, { status: 500 });
    }
}
